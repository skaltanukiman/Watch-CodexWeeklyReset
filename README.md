...
スクリプトは、Codex App Serverから取得した次回リセット時刻 `resetsAt`と

使用済み割合 `usedPercent`を前回保存した値と比較します。

現在の `resetsAt` と `usedPercent` が前回値より未来へ進んでいた場合、新しい週次使用枠へ切り替わったと判断します。

```text
前回のresetsAt    <  今回のresetsAt
                and
前回のusedPercent >  今回のusedPercent
                 ↓
          週次枠のリセットを検知
```
...
- トークンや週次枠等の仕様変更や、スクリプトの動作頻度等によって誤検知・誤通知する可能性もございます。
- 実際にトークンが回復しているかの正確な確認は、直接codexやchatgptアプリ等から行ってください